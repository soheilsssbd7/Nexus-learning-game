...
| ۶ | رابطِ کاربریِ فارسی/RTL | ✅ | CI سبز: **۳۷ فایل / ۳۱۵ تست / صفر شکست** ✓ — ممیزیِ کفِ لمسی روی هر ۹ صحنۀ UI + قاعدۀ «هر `concept_tag` ⇒ `skill.<tag>` در **هر دو** زبان» ✓ |
| ۷ | ۴۵ سطح + روایت + صوت | ✅ | CI سبز روی `e0aafca`: **۴۰ فایل / ۳۳۵ تست** ✓ · `validate_levels`: «۴۵ سطح · ۴۵/۴۵ قابل‌حل · قالب ۱۸ · UI ۹۳×۲ · بیت ۶ · صوت ۱۷» ✓ |
| ۸ | هنرِ نهایی (آواتار/بصری/جهان) | ✅ | CI سبز روی `d027b54` و `42b54bb`: **۴۷ فایل / ۴۳۴ تست** ✓ · سه قفلِ `check_typography`/`check_icon_assets`/`check_ui_string_i18n` ✓ · ۱۹ فایل بصری ≈۲۴۸KB زیرِ بودجۀ ۳۴۰۰KB ✓ |
| ۹ | بک‌اند + صفِ آفلایِن | ✅ | CI `5a12352`: **هر سه job سبز** ✓✓ — `GUT 4.7.2 · Scripts 48 · Tests 453 · 453 PASS · Orphans 1` · `backend`: ۹ step با **migrate دوبار روی Postgres 16 واقعی** + **۳۵/۳۵** `node:test` · `gdlint + content validation` با **۲۱ گیت** ✓ (پنج دورِ قرمز که همگی فقط در CI دیده شدند ✓§«فاز ۹») |
| ۱۰..۱۲ | QA/build اندروید · بتا · انتشار | ⬜ | — |
...
**۲-ز) رویدادِ «snapshotِ سندباکس» و قاعدۀ بازسازی ✓✓ (بار دوم، در همین فاز ۹ ✗✓):** محیطِ
اجرای Arena ممکن است میانهٔ session از صفر `git clone` شود ⇒ `.git`ِ تازه با `HEAD=90cdff6` و
**بدونِ کامیت‌های محلیِ push‌نشده** ✗✗ (reflog هم پاک ⇒ `git fsck --lost-found` خالی ✓). فایل‌های
دیسک می‌مانند ⇒ راهِ بازیابیِ قطعی همین است ✓✓: ① `cp -a game backend tools docs /tmp/rescueN/`
(بیمه) ② `git config --add remote.origin.fetch '+refs/heads/*:refs/remotes/origin/*'` (ریپازیتوری
تک‌شاخه clone می‌شود ✗✓ پس `origin/arena/…` اول **نیست**) ③ `git fetch` ④ `git reset --mixed
origin/arena/…` (هدیستوریِ ریموت + دیسکِ خودم ✓ بدونِ دست‌زدنِ فایل‌ها) ⑤ `git status` باید
**همان deltaی کامیت‌های گم‌شده** باشد (این‌بار ۱۱ ورودی ✓✓) ⑥ بازکامیت با نشانه‌هایِ حالتِ نهایی
(`grep -c` روی چند نشانۀ یکتا مثل `stats()["dropped"]`، `stamp += "Z"`، `check_backend_client_contract`
✓✗ نه با «احساسِ اینکه درست است») ⑦ `npm ci` دوباره (node_modules زیرِ ignore است ⇒ با snapshot
**می‌رود** و تست‌ها اول ۶ fail می‌شوند ✗✓) ⑧ push ✓. **قاعدۀ تازه: هر «بسته شد» قبل از رفتن به
بستهٔ بعدی push می‌شود** ✗✓ (کامیتِ محلی در این sandbox دادهٔ باارزش محسوب نمی‌شود؛ ۹ کامیتِ فاز ۹
یک‌بار سوخت و از دیسک زنده شد ✓✓).
...
### فاز ۹ — بک‌اند
**وضعیت: ۹.۱ ✅ · ۹.۲ ✅ · ۹.۳ ✅ · ۹.۴ ✅ · ۹.۵ ✅ (ADR-063) · ۹.۶ ✅ (ADR-064) ⇒ **فاز ۹ بسته شد ✓**

**اثباتِ CI (روی `5a12352` ✓✓ هر سه job سبز):** `Godot import + GUT (headless)` ⇒ **`GUT 4.7.2 · Scripts 48 · Tests 453 · Passing 453 · Failing 0 · Orphans 1 · 28.1s`** ✓✓ (`files_on_disk=48` ⇒ نه یک تستِ غایب ✗✓ قاعدۀ §۸.۴) · `backend` ⇒ ۹ step: `npm ci` قفل‌شده → `typecheck` → `build` + `test -f dist/src/db/schema.sql` → **migrate دوبار روی Postgres 16 واقعی** (بی‌ضرربودن ✓ DoD ۹.۲) → **۳۵/۳۵** تستِ `node:test` ✓ · `gdlint + content validation` ⇒ `validate_levels.py` سبز با **۲۱ گیت** ✓ (شش قفلِ تازهٔ همین فاز: پنج تا برای بک‌اند/قرارداد + `check_ci_yaml`) ✓✓ — پس DoD ۹.۱..۹.۶ **همه** در CI سنجیده شدند، نه محلی ✓ (سندباکس Godot ندارد ✗)

**پنج دورِ قرمزِ این فاز — و وجهِ تازه‌شان: هر پنج تا *فقط* در CI دیده شدند ✗✓** (۱) `name:`ِ کووت‌نشده با «: » ⇒ workflow parse نشد ⇒ **صفر job و صفر لاگ** ✗✗ ⇒ `check_ci_yaml` (سطح ۱ بی‌وابستگی + سطح ۲ `yaml.safe_load` با `pyyaml>=6` که در CI قفل شد ✓)؛ (۲) `ERR_CONNECTION_FAILURE` — میراث 3.x که در Godot 4 حذف شده ⇒ **Parse error** ⇒ GUT «۴۷ از ۴۸ فایل»؛ غایب ≠ قرمز ✗؛ (۳) `headers.join("|")` ⇒ در Godot 4 `join` متدِ *رشتۀ جداکننده* است (`sep.join(arr)`؛ `PackedStringArray` join ندارد ✗) ⇒ دو pattern در `PHANTOM_API` ✓ (۴) `FakeSender extends RefCounted` روی `var transport: Node` ⇒ خطای زمانِ اجرا در `before_each` ⇒ ۱۸ فیل زنجیره‌ای با پیام‌های بی‌معنا (`Out of bounds on Array`) ✗✓ — و اعترافِ صریح: این خانوادۀ خطا با regex گرفته نمی‌شود، CI تنها داورش است ✓؛ (۵) **باگِ واقعیِ production**: `_refresh_token()` شرط `if _player_id == ""` داشت و `_player_id` فقط با `sync_player_model` پر می‌شود ⇒ در سناریوی «نصبِ تازه: اول رویدادها، بعد مدل» پاسخِ ۴۰۱ فقط `auth_required` می‌داد و mint **بی‌صدا** رد می‌شد، یعنی صف تا ریستارت می‌خوابید ✗✓ (تستِ ۱۴ِ ۹.۶ گرفتنش ✓✓) ⇒ شناسه از خودِ صف خوانده می‌شود و اگر معتبر نبود `stats().last_error = "no_player_for_token"` + `Log.warn` ✓ (سکوتِ مدیریتِ خطا همان چیزی است که ADR-064 برایش نوشته شده ✓). ضمناً یک مورد از پنج دور، **تستِ بد** بود نه کدِ بد (خواندنِ `last_body()` پیش از `flush_once()` ✗✓) و همان‌جا سه سطرِ معنادارتر اضافه شد: `flush` ⇒ ۶، `sent.size()==1` (یک دسته، نه شش درخواست ✓§۹.۴)، `queue_size()==0` ✓ — `backend/` با Node 22 + Express + zod + `pg` ساخته شد؛ ۳۵ تست `node:test` (supertest + **pg-mem**) سبز ✓✓ و `GET /health` روی سرورِ درحال‌اجرا واقعاً ۲۰۰ داد ✓ (`npm run dev` ⇒ `listening_ports` ⇒ `curl` ✓✗ DoD ۹.۱ فقط با «لاگِ موفق» ثابت نمی‌شد: دورِ اول `main()` بعد از `listen` برمی‌گشت و `.then(process.exit)` سرور را ۰٫۹ ثانیه بعد می‌کُشت ✓✓ درس: «بالا آمدن» یعنی **پورتِ در گوش** ✓)
...
| تسک | خروجی | سنجش | وضعیت |
|---|---|---|---|
| ۱۰.۱ پوشش تستِ منطق (`grep -c '^def check_' tools/validate_levels.py` ⇒ ۲۲ تابع = ۲۱ گیتِ main + یک درون‌سطحی ✓) | گیتِ `check_test_coverage_map()`: هر فایلِ منطقیِ `scripts/{data,ai,gameplay,autoload}` باید در `game/tests/gut/*.gd` صریح نام برده شود ✓ (امروز: **۲۵/۲۵ ✓✓**)؛ پروب: `Orphan_probe.gd` بی‌تست ⇒ قرمز ✓✓ | «کمترینِ DoD ۱۰.۱» مکانیکی و ضدبازگشت شد ✓ — و چون سندباکس Godot ندارد ✗، **گزارش خطِ پوشش** معلق است: در CI باید `--coverage` روی Godot 4.7.2 آزموده شود و اگر شد، `*.gcov`/`coverage.xml` به artifact بنشیند ✓ (فعلاً ادعا نمی‌کنیم ✗) | ⚠ (گیت ✅ · پوشش عددی ⏳) |
| ۱۰.۲ `docs/playtest-protocol.md` | سندِ آماده‌ی جلسه: **۵ کار** («بدون کمک»، با جمله‌ی دقیقِ گفته‌شده) + چک‌لیست مشاهده + **ضبطِ صفحه‌محور و بی‌چهره** ✓§۹ + جدول ثبت + «بعد از جلسه: اول تستِ قرمز، بعد رفع» ✓ + قاعدۀ «پلی‌تست با `cloud_sync_enabled` خاموش» ✓ | هر سطر یا عدد دارد یا دستور ✓؛ کار ۲ مستقیماً `check_answer_leaks` را به چشمِ کودک گره می‌زند ✓ | ✅ |
| ۱۰.۵ چک‌لیست smoke (`docs/smoke-checklist.md`) | مسیرِ اصلی (Onboarding → ۳ سطح → آریا → داشبورد → بستن/بازکردنِ save ✓) + **بخش پلتفرم**: `targetSdk 36`/edge-to-edge/predictive back/فقط `INTERNET`+`VIBRATE`/`TotalTime<4s`/رشدِ PSS در ۶۰ دقیقه ✓ + **بخش آفلاین**: هواپیما ⇒ بی‌کرش، وصل ⇒ `queue_size()==0`، و `logcat` بی‌`IMEI/AAID/MAC/SSID/serial` ✓✓ | هر بند یا `adb`/`apkanalyzer` دارد یا عدد ✓؛ سه ادعای ساختگیِ خودم همان‌جا پاک شد ✗ (`display/window/handheld/safe_area` ← چنین کلیدی نیست ✓، `pushers` در `MasteryChart` ← اختراع ✓، نام Activity حدسی ← `resolve-activity` ✓) ⇒ قاعدۀ تازه: **در سندِ دستگاه‌محور هم نامِ API را از ابزار بخوان، نه از حافظه** ✓ | ✅ |
| ۱۰.۳ `AnalyticsManager` | `scripts/autoload/AnalyticsManager.gd` (autoload ✓ثبت‌شده بعدِ `NetworkClient` در `project.godot`) — ناظرِ `EventBus`، نه تولیدکننده ✓ (هر شش سیگنال از قبل در `GameState:58/75/229`، `LevelController:591/619`، `AriaController:128` می‌آیند ⇒ صفر تغییرِ رفتار در گیم‌پلی ✓) · `const HANDLERS` = تنها اعلامِ انواع ✓ · `build_event`/`flatten_payload` استاتیک و خالص ✓ · `sink: Node` تزریق‌پذیر ✓ (fake = `extends Node` ✗✓ درسِ ۹.۶) · `session_end` idempotent با `Window.focus_exited` + PREDELETE ✓✓ | **۱۸ تست** (`test_analytics_manager.gd`) ✓✗ **اثباتِ CI روی `41f7eae`: `GUT 4.7.2 · Scripts 49 · Tests 471 · Passing 471 · Failing 0 · Orphans 1` ✓✓** (`files_on_disk=49` ⇒ نه تستِ غایب ✓§۸.۴) — پیش از آن یک دور `470/471` و یک دور «۴۸ از ۴۹ فایل» داشت که **هر دو از خودِ تست بود، نه کد** ✗✓ و به دو گیتِ تازه تبدیل شدند (`check_duplicate_locals`، `check_multiline_string_concat`) ✓§ADR-065: شکلِ هر رویداد ✓، چهار کلیدِ §۵ و حذفِ `score` ✓، **متنِ راهنما هیچ‌جای رویداد نیست** ✓✓§۹، مرزِ `JsonScalar` (۲۰۰) ✓، payload تودرتو = افتِ **شمرده** ✓، `level_id` خارج از §۱ ⇒ کلید می‌افتد و رویداد می‌ماند ✓، پرچمِ والد خاموش ⇒ **هیچ رویدادی ساخته نمی‌شود** ✓، ردِ sink ⇒ `stats().rejected/reason` ✓ (بی‌سکوت ✓)، کلیدهای top-level ⊆ §۵ چون `EventSchema.strict()` است ✓، و «timestamp را ما نمی‌سازیم» ✓✓ · **سه قفلِ تازه:** بندِ ۴ `check_backend_client_contract` (هر نوعِ enumِ سرور باید در `HANDLERS` باشد ✓ پروب: حذفِ `error_occurred` ⇒ قرمز ✓) + `check_autoload_registration` (فایل ⟺ ثبت؛ پروب: `GhostMissing` و `Orphan_probe` ⇒ دو خطای درست ✓✓) + `check_test_coverage_map` (این فایل هم باید تست داشته باشد ✓) | ADR-065 ✓ | ✅ (اثباتِ device/`events` در `docs/smoke-checklist.md` §۳ ⏳=L3 ✓) |
| ۱۰.۴ `export_presets.cfg` + workflow خروجی | `game/export_presets.cfg` (دست‌نویس ✓ فقط کلیدهای مطمئن ✗ و به‌جای هر کلیدِ حدسی، کامنتِ تصمیم ✓ ADR-066): `use_gradle_build=true` (شرطِ لازمِ اعمالِ `target_sdk` ✗✓، نه انتخاب) · `min_sdk=24`/`target_sdk=36` ✓ ADR-009 · `include_filter="*.json"` ✓ ADR-036ِ **اصلاح‌شده** (کلیدِ اعلامیِ ADR-036 در Godot 4 وجود نداشت ✗✗ و همین تسک لو داد ✓) · `arm64-v8a` ✓ · `package/unique_name=com.nexus.balancerealm` ⚠ بازگشت‌ناپذیر ⇒ تأییدِ مالک قبل از ۱۱.۳ ✓ · `game/android/permissions.xml` (منبعِ مجوزها: فقط `INTERNET`+`VIBRATE` ✓§۹) · `.github/workflows/android-export.yml`: فقط `workflow_dispatch` ✓ ADR-004، JDK17 + platform 36 + build-tools 36.0.0 + قالب‌های export با کش، نصبِ template **سپس** درجِ مجوزها (ترتیب = تصمیم ✓ وگرنه overwrite بی‌صدا ✗✓)، `release` بی‌سه secret = ردِّ صریح (نه fallback به debug که «امضاشده» جا بزند ✓)، `apkanalyzer manifest print` روی **بستهٔ واقعی** ✓✓، گزارشِ بودجه در summary و artifact ۳۰ روز ✓ | **گیتِ `check_export_config()`** ✓: برابریِ عددِ پریست با **متنِ ADR-009** (جهتِ قفل: سند→کد ✓§۹.۲) · `include_filter` · قالبِ نامِ پکیج **و** اعلامش در ADR ✓ · هیچ مجوزِ حساسی (موقعیت/مخاطبین/دوربین/میکروفون/AAID/storage) ✓✗ و کامنت‌ها strip می‌شوند تا «فهرستِ چیزهایی که نمی‌خواهیم» گیت را قرمز نکند ✓ · هیچ رمزی در پریست ✓ADR-004 · **وجودِ `apkanalyzer` در workflow** ✓ (تا لایۀ (ب) بی‌صدا حذف نشود ✓✓) · «نبودِ فایلِ سند» هم خطاست، نه skip ✗✓ (گیتِ بی‌سنجشِ سبز = بدترین نوع ✓ این باگ را خودِ همین گیت در دورِ اول داشت ✓✓) · پروبِ چهارخروبی ⇒ **شش** خطای درست ✓✓ | ⚙ (پیکربندی ✅ در مخزن · **ساختِ واقعی** = اجرای دستیِ workflow + گوشیِ تو ⏳ DoDِ L3 ✓ · AAB-by-extension هم در اولین اجرا تأیید می‌شود ⏳ نه ادعا ✓) |

**وضعیت فاز ۱۰:** ۱۰.۲ ✅ · ۱۰.۳ ✅ (۱۸ تست · **CI 471/471 ✓✓** · ADR-065) · ۱۰.۴ ⚙ (پیکربندی + workflow + `check_export_config` ✅ ✗ ساختِ واقعی و نصبِ دستگاه ⏳=L3 ✓) · ۱۰.۵ ✅ ⇒ **فاز ۱۰ در مخزن بسته شد ✓** (سه ⬜ باقی‌مانده بیرون از مخزن‌اند: اجرای دستیِ workflow، گوشیِ واقعی، و `--coverage` که ساقط شد ✓) · ۱۰.۱ ✅ **با بازتعریفِ صریح** ✓✗ `--coverage` در Godot 4.7.2 آزموده شد و موتور گفت `Unknown arguments: ["--coverage"]` ⇒ «درصد پوشش» در این ریپو **اعلام نمی‌شود** ✓✗ جایش `check_test_coverage_map()` = **۲۶/۲۶ فایلِ منطقی با تست** ✓ + junit/خلاصۀ GUT در artifact ✓ (ADR-067) — **پنج قفلِ تازه** در `tools/validate_levels.py` از این فاز درآمد: بندِ ۴ قراردادِ دوزبانه · `check_autoload_registration` · `check_duplicate_locals` · `check_multiline_string_concat` · `check_export_config` ✓✓ (همه با پروبِ قرمز/سبزِ عمدی ✓) — و سه بندِ باقی‌مانده به دو چیز بیرونی گره‌اند: **Godot برای `--coverage`** (فقط CI) و **دستگاهِ واقعی برای نصب/AAB** (فقط تو ✓✓ DoD ۰.۲/۱۰.۴ همین را می‌خواهد) ✗.
۱۰.۱ پوشش تست (۱۰۰٪ منطق حیاتی) · ۱۰.۳ `AnalyticsManager` (۶ رویداد) · ۱۰.۴ `export_presets.cfg` (۱۰.۴ `export_presets.cfg` (minSdk 24 → **target/compile 36**، `INTERNET` + `VIBRATE` فقط، icon، AAB) · ۱۰.۵ smoke checklist.
...
| تسک | خروجی | سنجش | وضعیت |
|---|---|---|---|
| ۱۱.۱ سیاست حریم خصوصی (fa + en) | `docs/privacy-policy-fa.md` + `docs/privacy-policy-en.md` ✓ (فهرستِ «جمع می‌شود»: ۵ قلم ✓ / «هرگز»: ۱۶ قلم ✓ / سه کنترلِ والد ✓ / نگهداری ۱۲ ماه ✓ / **سه چیز که در این نسخه نیست ⚠** ✓) | گیتِ `check_privacy_policy()` ✓✓: بخش‌محور + نفی‌محور + **برابریِ شش نوعِ رویدادِ اعلامی با enumِ zod** ✓ + هم‌تعدادیِ قلم‌های «هرگز» در دو زبان ✓ + اجبارِ «retention» و «نشانیِ تماس» ✓✗ پروب‌ها: حذفِ `MAC` از fa ⇒ `fa=15 · en=16` قرمز ✓ / «ایمیل» بدون نفی در بخش جمع‌آوری ⇒ قرمز ✓ / حذفِ `session_start` از سیاست ⇒ قرمز ✓✓ | ✅ (میزبانیِ وب و فرمِ حذف ⏳=تسک ۱۲.۴ ✓) |
| ۱۱.۲ چک‌لیست انطباق Families | `docs/families-compliance.md` ✓ — ده سطرِ «چه چیزی نمی‌گیریم» با **اثر/دستورِ راستی‌آزمایی در مخزن** ✓ (نه ادعای انتزاعی) + پاسخ‌های Data Safety + §۴ فنی (`targetSdk 36` ✓ `minSdk 24` ✓ `exclude_filter` برای GUT/tests ✓) + جدولِ امضای §۵ | هر سطر یا گیت دارد یا تست یا دستور `apkanalyzer` ✓؛ سه ⬜ باقی‌مانده **بیرون از مخزن** اعلام شده (IARC، هاست، مسیرِ حذفِ انسانی ✗§۱۲.۴) ⇒ DoD ۱۱.۲ «امضا پیش از ارسال build» عمداً ⬜ مانده ✓✗ جا نمی‌زنیم تمام شده ✓ | ⚙ |
| ۱۱.۳ Closed testing (۱۰۰ خانواده ⇒ ≥۱۰ تستر) | ⬜ — بیرون از مخزن ✓ (Console + دعوت‌نامه) | DoD: ≥۱۰ نصب موفق ⇒ **بعد از** بسته شدنِ ۱۲.۴ ✓ (پیش از آن، «درخواست حذف» وجود ندارد ✗=ریسکِ سیاست) | ⬜ |
| ۱۱.۴ فرم بازخورد + PauseMenu link | ⬜ — کدش کوچک است (یک `LinkButton` پشت قفل والدین ✓§۶.۵) ولی **میزبانِ فرم** لازم دارد ⇒ ۱۲.۴ | «حداقل یک بازخورد واقعی از تستر بیرونی» ⏳ | ⬜ |

**وضعیت فاز ۱۱:** ۱۱.۱ ✅ · ۱۱.۲ ⚙ (چک‌لیست تکمیل شده ✓ امضا ⬜) · ۱۱.۳/۱۱.۴ ⬜ — وجهِ این فاز این است که
سه بندِ ⬜ با کد بسته نمی‌شوند ✗✓ (فرم فروشگاه، هاست، آدمِ واقعی) ⇒ هر سه در همین جدول **به‌نامِ خودشان**
ثبت شده‌اند، نه پنهان پشت یک ✅ کلی ✓ (ADR-067 ✓)

### فاز ۱۲ — انتشار (افزوده‌ی من؛ A17)
| تسک | خروجی | سنجش | وضعیت |
|---|---|---|---|
| ۱۲.۱ گرافیکِ فروشگاه (icon 512 · feature 1024×500 · ۸ اسکرین‌شات · ویدیو ۳۰s) | ⬜ | اسکرین‌شات باید **رندرِ واقعی** باشد ✗✓ و موتور در این سندباکس/CIِ متنی اجرا نمی‌شود ⇒ از اولین اجرای APK روی دستگاه گرفته می‌شود ✓ (۱۰.۴ ⚙) | ⬜ |
| ۱۲.۲ متن لیستینگ fa+en (≤۳۰ / ≤۸۰ / ≤۴۰۰۰) | `docs/store-listing.md` ✓ دو زبان + «چیزی که **نوشته نشده**» (ادعای «تضمین نمره» ✗✓ چون شواهد نداریم ✓) | گیت `check_store_listing()` ✓✓: کاراکترشماریِ واقعی (نه «کم‌وبیش») + سه ادعای تبلیغ‌نداشتن/IAP‌نداشتن/آفلاین + لینکِ سیاست + **برابریِ ردهٔ سنی با سیاست و سایت** ✓✗ پروب‌ها: عنوان ۳۴ ✓ / کوتاه ۹۶ ✓ / حذف «آفلاین» ✓ / سنِ دوشاخه ✓✓ | ✅ |
| ۱۲.۳ فرم‌های Play Console (Data Safety · IARC · Target audience · Ads=none · Pricing) | ⬜ بیرون از مخزن ✓ | پاسخ‌های آماده‌اش در `docs/families-compliance.md` §۳ است ✓✗ خودِ فرم، مالک ⚠ | ⬜ |
| ۱۲.۴ Pages (پشتیبانی + حریم خصوصی) | `site/**` + `site_src/**` + `tools/build_site.py` ✓✓ (۸ صفحۀ fa/en ✓ خانه، والدین، پشتیبانی، بازخورد، نسخۀ وب، ۴۰۴، سیاست fa+en **از docs رندر می‌شود** ✓) + ریشۀ مخزن = `index.html` ناحیۀ ورود ✓ | `check_site()` ✓✓ پنج قفل (تازگیِ تولید ✓ پیوندِ شکسته ✗ CDN/`<script>` ✗✓ **placeholderِ بی‌علامت** ✗ سقفِ ۹MB فایل ✓ پالت/قرمزِ §۲ ✓ `<p>` داخل لیست ✗) ✓ پروب‌ها: drift ✓ / `nope.html` ✓ / fonts.googleapis ✓ / `<script>` ✓ / `#FF0000` ✓ / حذفِ رنگ ✓ / ۹٫۵MB ✓ / حذفِ 404.html ✓✓ | ✅ **منتظرِ تأییدِ انتشارِ Pages** ✓ (URL: `https://soheilsssbd7.github.io/Nexus-learning-game/`) |
| ۱۲.۵ نسخه‌بندی/CHANGELOG/rollout | `VERSION` = تنها منبع ✓ + `CHANGELOG.md` ✓ (Added/Changed/Fixed/**Not done ⚠**) ✓ workflowِ اندروید `version/name` را از همان می‌خواند ✓ | گیت: `VERSION` semver ✓ `check_site` عددِ `{{VERSION}}` را در footer می‌سنجد ✓ | ⚙ (rollout ۱۰→۵۰→۱۰۰٪ در Console ⬜) |
| ۱۲.۶ قیمت و مدل درآمد | مدل قطعی: پولیِ یک‌باره، بدون IAP/اشتراک ✓ (ADR-027 ✓ بازتاب در لیستینگ و سیاست ✓) · **عددِ قیمت ⬜ مالک** | «تبلیغ/IAP نیست» در لیستینگ + `check_export_config` (SDK تبلیغاتی ممنوع ✓) + `exclude_filter` ✓ | ⚙ |

**وضعیت فاز ۱۲:** ۱۲.۲ ✅ · ۱۲.۴ ✅ · ۱۲.۵ ⚙ · ۱۲.۶ ⚙ · ۱۲.۱/۱۲.۳ ⬜ — و **گلوگاهِ واقعیِ انتشار** همان
سه ⬜ نیست ✗✓ بلکه دو چیز است: (۱) یک بار اجرای `android-export.yml` + نصب روی گوشی (۱۰.۴ ⚙) ✓✗ چون بدونِ
آن نه اسکرین‌شاتِ واقعی داریم نه ۱۲.۱ ✓ (۲) آدرسِ انسانیِ پشتیبانی/حذفِ داده (`<host>` در سیاست و سایت ⚠) ✓✗
که بدون آن Data Safety صادقانه پر نمی‌شود ⇒ **پیش از این دو، «آمادهِ انتشار» نگفته‌ایم** ✓

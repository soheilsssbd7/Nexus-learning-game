...
**جمع محتوا:** **۴۵ سطح از ≈۴۵ ✓ کامل** (Tier ۱: ۵ · Tier ۲: ۱۰ · Tier ۳: ۱۰ · Tier ۴: ۱۰) · Elo پیوسته ۹۰۰→۱۸۸۰ · `validate_levels.py`: «قابل‌حل تأییدشده: ۳۵/۳۵».
...
**باقیِ فاز ۷:** ۷.۵ `story_beats.json` · ۷.۶ `AudioManager` · ۷.۷ «CI evidence».
...
⚠️ **وضعیت اتصال (این دور، ۱۴۰۵-۰۶-۲۰):** سندباکس `.git` را بازسازی کرد ⇒ تاریخچهٔ محلی روی
`90cdff6` نشست و `gh api` هم `401 Bad credentials` داد ✗✗ کارها **در درختِ کار سالم و committed
محلی‌اند** (`c855a43` ابزار · `871add6` محتوا · این کامیت اسناد ✓) و ریموت هنوز روی `3a79b97`
(سبز ✓) است ⇒ دستورالعملِ بازیابی (با ترتیبِ درستِ flagها ✗✓ این را یک‌بار اشتباه نوشتم و اصلاح شد):
۱) `git fetch origin arena/01a07efb-nexus-learning-game` · ۲) `git reset --mixed FETCH_HEAD`
(**`--mixed` نه `--soft`** ✗✗ چون index محلی فقط `tier5/tools/docs` را می‌شناخت و با `--soft`،
`git status` کل `game/` را «حذف‌شده» نشان می‌داد ⇒ یک `commit -a` فاجعه می‌ساخت ✗✓ `--mixed`
index را به درختِ ریموت تنظیم می‌کند و درختِ کار دست‌نخورده می‌ماند ✓) · ۳) `git diff --stat
FETCH_HEAD -- game/` باید **خالی** باشد ✓ (اثباتِ اینکه موتور و تست‌ها همان ۷.۳ هستند ✗) ·
۴) `git add` با مسیرهایِصریح (`game/data/levels/tier5 tools/validate_levels.py docs/05 docs/06`)
و سه کامیتِ تمیز روی نوکِ ریموت ✓ · ۵) push ✓ و بعد چک‌کردنِ CI ✓
**هیچ‌وقت force-push نکن** (تاریخچهٔ ریموت منبعِ حقیقت است ✓) و **هرگز `git add -A`** (درختِ
محلی ناکاملاً ردیابی‌شده است ✗✓ مسیرِ صریح = قانونِ همیشگیِ این ریپو).

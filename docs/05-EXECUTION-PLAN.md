...
**جمع محتوا:** ۲۵ سطح از ≈۴۵ (Tier ۱: ۵ · Tier ۲: ۱۰ · Tier ۳: ۱۰) · Elo پیوسته ۹۰۰→۱۶۸۰ · `validate_levels.py`: «قابل‌حل تأییدشده: ۲۵/۲۵».
**بدهیِ تازه:** (۱) کره‌های `fixed`/شبح قابل کشیدن‌اند ⇒ تصمیمِ owner برای قفلِ جابه‌جایی (ADR-054 «پیامد الف»)، (۲) `scales` در فایل/اسکیمای `LevelData` قبل از ۷.۳، (۳) `ghost` در باتِ محتوا فعلاً از راه `target_value` سنجیده می‌شود؛ اگر Tier ۴/۵ «دو کفه» آمد، بات هم `scales` را باید بسنجد.
...
باقیِ فاز: ۷.۲ Tier 3 (کسر + `ghost_orbs`) · ۷.۳ Tier 4 (`scales` در فایل و در اسکیمای `LevelData` ✗
بدهیِ ثبت‌شده) · ۷.۴ Tier 5 · ۷.۵ `story_beats.json` · ۷.۶ `AudioManager`.

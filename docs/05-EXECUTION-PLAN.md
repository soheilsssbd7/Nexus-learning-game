...
| ۱۰.۳ `AnalyticsManager` | — (۶ رویداد §۵ در نقاطِ درست trigger و بفرست به `NetworkClient` ✓) | DoD: «یک جلسۀ کامل ⇒ همه رویدادها در جدول `events`» — نیازمند `backend`ِ بالا ✓ و تصمیمِ merge مدل (ADR-064 §د) | ⬜ |
...
**وضعیت فاز ۱۰:** ۱۰.۲ ✅ · ۱۰.۵ ✅ (مستندات، بی‌ریسکِ CI ✓) · ۱۰.۱ ⚠ (گیتِ نقشه ✅، عددِ پوشش ⏳) · ۱۰.۳/۱۰.۴ ⬜ — و سه بندِ باقی‌مانده به دو چیز بیرونی گره‌اند: **Godot برای `--coverage`** (فقط CI) و **دستگاهِ واقعی برای نصب/AAB** (فقط تو ✓✓ DoD ۰.۲/۱۰.۴ همین را می‌خواهد) ✗.
